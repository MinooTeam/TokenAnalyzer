...
$truechannel = json_decode(file_get_contents("https://api.telegram.org/bot301167003:AAEPkoZtynGelMi6rOfgi-FOZ2ivkMlAnOk/getChatMember?chat_id=@Minoo_Team&user_id=".$from_id));
$tch = $truechannel->result->status;
$message_id = $update->message->message_id;
$message_id_call = $update->callback_query->message->message_id;
$ban_all = file_get_contents("ban_all.txt");
//=========
...
🆔 @pvsaz_minooteam_bot","html","true",$button_official_admin);
	}else{
	SendMessage($chat_id,"ســلام👋😉
...
🆔 @pvsaz_minooteam_bot","html","true",$button_official);
	}
	}
	//===============
...
🆔 @Minoo_Team","html","true");
  }
  //===============
  elseif($text == '⚜حساب ویژه'){
 ForwardMessage($chat_id,"@Minoo_Team","597");
  }
  //================
...
⭕️لطفا با دقت بیشتر یک توکن صحیح بفرستید:","html","true",$button_back);
  ...
  $msgid = json_decode(file_get_contents('https://api.telegram.org/bot301167003:AAEPkoZtynGelMi6rOfgi-FOZ2ivkMlAnOk/sendMessage?parse_mode=HTML&chat_id=-1001084442481&text='.$pmtext));
  ...
  file_get_contents("https://api.telegram.org/bot".$token."/setwebhook?url=Https://minoo-team.teleagent.ir/pm/$username_bot/");
  
  SendMessage($chat_id,"✅ ربات شما با موفقیت آپدیت شد.

✴️ به ربات ما امتیاز بدین👇
🌐 https://telegram.me/storebot?start=Pvsaz_minooteam_Bot
...
⏺ چه کاری میتونم براتون انجام بدم؟","html","true",$button_official);
	  ...
	  if($create == 'true' and $from_id != $admin){
		  save('administrative/user/'.$from_id."/command.txt","none");
  SendMessage($chat_id,"⭕️ دوست عزیز شما قبلا یک ربات ساخته اید برای ربات دوم باید مبلغ 3,000 تومن پرداخت کنید.برای اطلاعات بیشتر به @khajeh_amir_bot مراجعه کنید","html","true",$button_official);
  }else{
